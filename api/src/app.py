...
from flask import g, Flask, abort, flash, redirect, request, send_file, session, url_for
...
app = Flask(__name__, template_folder=str(FRONTEND_DIR / 'templates'),
            static_folder=str(FRONTEND_DIR / 'static'), static_url_path='/static')
...
def security_headers(response):
    ...
    style_nonce = getattr(g, 'style_nonce', None)
    style_policy = "'self'" + (f" 'nonce-{style_nonce}'" if style_nonce else "")
    response.headers['Content-Security-Policy'] = f"default-src 'self'; img-src 'self' blob: https:; style-src {style_policy}; script-src 'self'; frame-ancestors 'none'; form-action 'self'; base-uri 'self'"
