...
class ApplicationTests(unittest.TestCase):
    ...
    def test_car_profile_create_edit_and_link_request(self):
        ...
        self.assertEqual(page_data(form)['selected_car_id'], car_id)
        ...
        self.assertEqual(page_data(self.client.get(response.location))['car']['specs']['rear_brakes'], 'Drums')
    ...
    def test_car_photo_upload_keep_replace_remove_and_invalid(self):
        ...
        self.assertEqual(len(page_data(self.client.get(car_url + '/edit'))['car']['photo_ids']), 1)
        self.assertTrue(next(c for c in page_data(self.client.get('/cars'))['cars'] if c['id'] == car_url.rsplit('/', 1)[-1])['photo_ids'])
    ...
    def test_car_gallery_upload_and_homepage(self):
        ...
        self.assertEqual(len(next(c for c in page_data(page)['cars'] if c['id'] == car_id)['photo_ids']), 7)
        for row in photos:
            url = f'/cars/{car_id}/photos/{row["id"]}'
            self.assertIn(str(row['id']), next(c for c in page_data(page)['cars'] if c['id'] == car_id)['photo_ids'])
    ...
    def test_edit_photos_and_validation_are_atomic(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertIn(removed, page_data(response)['removed'])
