...
- O transporte Codex existente foi parametrizado com prompt/schema próprios, mantendo por omissão o comportamento de pesquisa. O transporte Open WebUI de enriquecimento usa o endpoint de completions estruturadas sem ferramentas em `part_enrichment.py`; a pesquisa de anúncios mantém o seu transporte com ferramentas e histórico remoto.
...
Para executar testes, usar uma base de dados PostgreSQL isolada com `db/init.sql` aplicado e as dependências do projeto:

```sh
python -m unittest discover -s tests
node tests/test_part_profile_merge.js
```
