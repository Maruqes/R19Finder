...
Aplicação Flask/Jinja/PostgreSQL, com JavaScript local e serviços Docker separados. `app.py` cria/edita pedidos e normaliza fotografias; `ai.py` configura Codex/Open WebUI; `search.py` contém execução dos fornecedores e prompts de pesquisa; `scheduling.enqueue_search` centraliza snapshots; `db/init.sql` contém migrações idempotentes executadas pelo mecanismo existente.
...
Criar serviço `enrichment_worker.py` e serviço Compose correspondente, reutilizando ligações, catálogo, autenticação e normalização existentes. Separação evita que uma pesquisa longa de anúncios bloqueie a revisão de uma peça; limitar concorrência total por fornecedor, sobretudo sessões Codex, antes de ativar execução simultânea.
